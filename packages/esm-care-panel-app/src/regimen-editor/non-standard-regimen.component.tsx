...
const NonStandardRegimen: React.FC<NonStandardRegimenProps> = ({
  ...
  const matchingCategory = standardRegimen.find(
    (item) => item.categoryCode === "ARV"
  ); // Non standard regimen will be exclusively for ARVs
        ...
        {selectedRegimenType === "nonStandardUuid" ? (
          ...
          >
            {!selectedRegimenLine || selectedRegimenLine == "--" ? (
              <SelectItem
                text={t("selectRegimenLine", "Select Regimen Line")}
                value=""
              />
            ) : null}
            {matchingCategory?.category.map((line) => (
              <SelectItem
                key={line.regimenline}
                text={line.regimenline}
                value={line.regimenLineValue}
              >
                {line.regimenline}
              </SelectItem>
            ))}
          ...
          <div>
            {selectedRegimens.map((selectedRegimen, index) => {
              const availableRegimens = nonStandardRegimen.filter(
                (regimen) =>
                  !selectedRegimens.includes(regimen.uuid) ||
                  regimen.uuid === selectedRegimen
              );
