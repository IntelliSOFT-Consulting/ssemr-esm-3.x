...
const ProgramSummary: React.FC<ProgramSummaryProps> = ({ patientUuid, programName }) => {
  ...
  // const { regimenEncounter, error: regimenError } = useRegimenEncounter(patientUuid, '23322fd6-3dbb-410e-8bee-6210dfcd5f71');

  // if (regimenError) {
  //   return <span>{t('regimenError', 'Error fetching regimen:')}</span>;
  //   console.error('Error fetching regimen:', regimenError);
  // }
              ...
              <div className={styles.card}>
                ...
                <div className={styles.container}>
                  ...
                  <div className={styles.content}>
                    ...
                    <p>
                      <span className={styles.value}>{data?.cd4 ? data?.cd4 : '--'}</span>
                      {data?.cd4Date ? <span> ({formatDate(new Date(data?.cd4Date))}) </span> : ''}
                    </p>
                  </div>
                  <div className={styles.content}>
                    <p>{t('CD4Percentage', 'CD4 percentage')}</p>
                    <p>
                      <span className={styles.value}>{data?.cd4Percent ? data?.cd4Percent : '--'}</span>
                      {data?.cd4PercentDate ? (
                        <span>({formatDate(new Date(data?.cd4PercentDate))}) </span>
                      ) : (
                        ''
                      )}
                    </p>
                  ...
                  <div className={styles.content}>
                    ...
                    <p>
                      <span className={styles.value}>
                        {t('whoStage', 'WHO STAGE')} {data?.whoStage ? data?.whoStage : '--'}
                      </span>
                      {data?.whoStageDate ? <span> ({formatDate(new Date(data?.whoStageDate))}) </span> : ''}
                    </p>
                  </div>
                  <div className={styles.content}>
                    <p>{t('regimen', 'Regimen')}</p>
                    <p className={styles.value}>
                      {data?.lastEncDetails?.regimenShortDisplay
                        ? data?.lastEncDetails?.regimenShortDisplay
                  ...
                  <div className={styles.content}>
                    ...
                    <p className={styles.value}>
                      {data?.lastEncDetails?.startDate
                        ? formatDate(new Date(data?.lastEncDetails?.startDate))
                        : '--'}
                    </p>
                  </div>
                </div>
