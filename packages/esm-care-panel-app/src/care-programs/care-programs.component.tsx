...
const CarePrograms: React.FC<CareProgramsProps> = ({ patientUuid }) => {
  ...
  const { carePrograms, isLoading, isValidating, error } =
  ...
  const handleCareProgramClick = useCallback(
    (careProgram: PatientCarePrograms) => {
      ...
      const formUuid = isEnrolled
        ? careProgram.discontinuationFormUuid
        : careProgram.enrollmentFormUuid;
      ...
      currentVisit
        ? launchPatientWorkspace("patient-form-entry-workspace", {
            ...
            formInfo: {
              ...
              formUuid,
              additionalProps:
                { enrollmenrDetails: careProgram.enrollmentDetails } ?? {},
            },
          })
        : launchStartVisitPrompt();
    },
          ...
          status: (
            <div className={styles.careProgramButtonContainer}>
              <span>
                {capitalize(
                  `${careProgram.enrollmentStatus} ${
                    ...
                    careProgram.enrollmentStatus === "active"
                      ? `Since (${formatDate(
                          new Date(careProgram.enrollmentDetails.dateEnrolled)
                        )})`
                      : ""
                  }`
                )}
              ...
              <Button
                ...
                renderIcon={
                  careProgram.enrollmentStatus == "active" ? Close : DocumentAdd
                }
              >
                {careProgram.enrollmentStatus == "active"
                  ? "Discontinue"
  ...
  if (error) {
    return (
      <ErrorState
        headerTitle={t("errorCarePrograms", "Care programs")}
        error={error}
      />
    );
  }
  ...
  if (carePrograms.length === 0) {
    return (
      <EmptyState
        headerTitle={t("careProgram", "Care program")}
        displayText={t("careProgram", "care program")}
      />
    );
  }
